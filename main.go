...
func ParseFlags(opts *ffuf.ConfigOptions) *ffuf.ConfigOptions {
	...
	flag.StringVar(&opts.Filter.Status, "fc", opts.Filter.Status, "Filter HTTP status codes from response. Comma separated list of codes, ranges, and regex patterns (e.g., /pattern/i).")
	...
	flag.StringVar(&opts.General.UserAgentFile, "ua-file", opts.General.UserAgentFile, "File containing User-Agent strings, one per line. Randomly rotated for each request.")
	...
	flag.StringVar(&opts.Matcher.Status, "mc", opts.Matcher.Status, "Match HTTP status codes, ranges, or regex patterns (e.g., /pattern/i). Use \"all\" for everything.")
	...
	flag.StringVar(&opts.Output.OutputFormat, "of", opts.Output.OutputFormat, "Output file format. Available formats: json, jsonl, ejson, html, md, csv, ecsv (or, 'all' for all formats)")
